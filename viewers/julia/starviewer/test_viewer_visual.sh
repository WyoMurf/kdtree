...
# Automated visual smoke test for the Julia starviewer, same convention
# as viewers/go/starviewer and viewers/rust/starviewer's own copies: run
# it against a real star-catalog directory with the
# SV_CAM_*/SV_TARGET_*/SV_SCREENSHOT env vars (see main.jl) to drive the
# camera to a known view and capture a screenshot, without a human at the
# keyboard. This doesn't assert anything about the *contents* of the
# screenshot -- it's a smoke test (does it run, does it produce a
# non-empty image), not a pixel-diff regression test. Look at the output
# PNG yourself after running this.
#
...
# seconds each (see shard.jl's Shard/ensure_shard_loaded! comment) -- this
...
OUT_DIR="$CATALOG_DIR/visual_test_output_julia"

echo "=========================================================="
echo " starviewer (Julia) visual smoke test"
echo "=========================================================="
...
(
    cd "$CATALOG_DIR"
    rm -f "$png"
    SV_SCREENSHOT="$png" SV_SCREENSHOT_FRAME=30 timeout 120 julia --project="$JULIA_PROJECT_DIR" "$SCRIPT_DIR/main.jl" >/dev/null 2>&1 || true
