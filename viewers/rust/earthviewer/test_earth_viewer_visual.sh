...
# Automated visual smoke test for the Rust earthviewer, mirroring
# C/test_earth_viewer_visual.sh and viewers/go/earthviewer's own copy:
# build it, run it a few times against a real city-data directory with the
# EV_LON/EV_LAT/EV_ALT/EV_SCREENSHOT env vars (see main.rs) to drive the
...
# of the screenshots -- it's a smoke test (does it build, does it run, does
# it produce non-empty images), not a pixel-diff regression test. Look at
# the output PNGs yourself after running this.
#
...
OUT_DIR="$CITY_DATA_DIR/visual_test_output_rust"
BIN="$SCRIPT_DIR/../target/debug/earthviewer"

echo "=========================================================="
echo " earthviewer (Rust) visual smoke test"
echo "=========================================================="
...
echo "[BUILD] cargo build -p earthviewer"
(cd "$SCRIPT_DIR/.." && cargo build -p earthviewer --quiet)

mkdir -p "$OUT_DIR"

# name, lon, lat, altitude(km), frame-to-capture-at -- same three views as
# the C smoke test and the Go port's, for a like-for-like comparison.
VIEWS=(
    "default::::30"
