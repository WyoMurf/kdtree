...
# Automated visual smoke test for the Go starviewer, same convention as
# earthviewer/test_earth_viewer_visual.sh: build it, run it against a real
# star-catalog directory with the SV_CAM_*/SV_TARGET_*/SV_SCREENSHOT env
# vars (see main.go) to drive the camera to a known view and capture a
...
# seconds each (see shard.go's Shard/EnsureShardLoaded comment) -- this
...
OUT_DIR="$CATALOG_DIR/visual_test_output_go"
BIN="$SCRIPT_DIR/starviewer.bin"

echo "=========================================================="
echo " starviewer (Go) visual smoke test"
echo "=========================================================="
...
echo "[BUILD] go build ./starviewer"
(cd "$SCRIPT_DIR/.." && go build -o "$BIN" ./starviewer)
