...
# Automated visual smoke test for the Go earthviewer, mirroring
# C/test_earth_viewer_visual.sh: build it, run it a few times against a real
# city-data directory with the EV_LON/EV_LAT/EV_ALT/EV_SCREENSHOT env vars
# (see main.go) to drive the camera to a few known views and capture a
# screenshot of each, without a human at the keyboard. This doesn't assert
# anything about the *contents* of the screenshots -- it's a smoke test
# (does it build, does it run, does it produce non-empty images), not a
# pixel-diff regression test. Look at the output PNGs yourself after running
# this.
#
...
OUT_DIR="$CITY_DATA_DIR/visual_test_output_go"
BIN="$SCRIPT_DIR/earthviewer.bin"

echo "=========================================================="
echo " earthviewer (Go) visual smoke test"
echo "=========================================================="
...
echo "[BUILD] go build ./earthviewer"
(cd "$SCRIPT_DIR/.." && go build -o "$BIN" ./earthviewer)

mkdir -p "$OUT_DIR"

# name, lon, lat, altitude(km), frame-to-capture-at -- same three views as
# the C smoke test, for a like-for-like comparison.
VIEWS=(
    "default::::30"
