...
func TestFloat64Basic(t *testing.T) {
	tree := Create[float64]()
	box1 := Box[float64]{0.0, 0.0, 10.5, 10.5}
	box2 := Box[float64]{20.25, 20.25, 30.75, 30.75}
	box3 := Box[float64]{5.5, 5.5, 15.5, 15.5}

	tree.Insert("item1", box1)
	tree.Insert("item2", box2)
	tree.Insert("item3", box3)

	if count := tree.Count(); count != 3 {
	...
	searchArea := Box[float64]{0.0, 0.0, 12.0, 12.0}
	gen := tree.Start(searchArea)
	found := 0
	for {
		item, _, ok := gen.Next()
	...
	list := tree.Nearest(20.25, 20.25, 1)
...
func TestSerializeFloat64(t *testing.T) {
	...
	tree.Insert("item1", Box[float64]{1.5, 1.5, 1.5, 1.5})
	tree.Insert("item2", Box[float64]{-3.25, -3.25, -3.25, -3.25})
	tree.Insert("item3", Box[float64]{100.125, 100.125, 100.125, 100.125})

	bounds, err := tree.GetBounds()
	if err != nil {
		t.Fatalf("GetBounds failed: %v", err)
	}
	expectedBounds := Box[float64]{-3.25, -3.25, 100.125, 100.125}
	if bounds != expectedBounds {
		t.Fatalf("Expected bounds %v, got %v", expectedBounds, bounds)
	}
