...
func randBox(rng *rand.Rand) Box {
	var b Box
	b[Left] = int64(rng.Intn(RANGE_SPAN) + MIN_RANGE)
	b[Bottom] = int64(rng.Intn(RANGE_SPAN) + MIN_RANGE)
	b[Floor] = int64(rng.Intn(RANGE_SPAN) + MIN_RANGE)
	b[Right] = b[Left] + int64(rng.Intn(BOX_RANGE))
	b[Top] = b[Bottom] + int64(rng.Intn(BOX_RANGE))
	b[Ceil] = b[Floor] + int64(rng.Intn(BOX_RANGE))
	return b
}
...
func TestKD3D(t *testing.T) {
	...
	box1 := Box{0, 0, 0, 10, 10, 10}
	box2 := Box{20, 20, 20, 30, 30, 30}
	box3 := Box{5, 5, 5, 15, 15, 15}
...
func TestSoftDelete(t *testing.T) {
	...
	// Verify empty
	gen := tree.Start(Box{MIN_RANGE - 1, MIN_RANGE - 1, MIN_RANGE - 1, MAX_RANGE + 1, MAX_RANGE + 1, MAX_RANGE + 1})
...
func TestHardDelete(t *testing.T) {
	...
	for i := KD_BOXES - 1; i >= 0; i-- {
		if !tree.HardDelete(i+1, boxes[i]) {
			// Try to find it first to see if it's actually missing or search failed
...
func TestNearest(t *testing.T) {
	...
	for m := 1; m <= 20; m *= 2 {
		for q := 0; q < 50; q++ { // 50 queries instead of 100 for speed
			qx := int64(rng.Intn(RANGE_SPAN) + MIN_RANGE)
			qy := int64(rng.Intn(RANGE_SPAN) + MIN_RANGE)
			qz := int64(rng.Intn(RANGE_SPAN) + MIN_RANGE)
