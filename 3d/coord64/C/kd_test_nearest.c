 ...
 */
static double box_dist(long long qx, long long qy, long long qz, kd_box box)
{
...
int main(int argc, char **argv)
{
	...
	for (q = 0; q < NUM_QUERIES; q++) {
	    long long qx = (rand() % RANGE_SPAN) + MIN_RANGE;
	    long long qy = (rand() % RANGE_SPAN) + MIN_RANGE;
	    long long qz = (rand() % RANGE_SPAN) + MIN_RANGE;
	    int visited;
	...
	int qz = (boxes[0][KD_FLOOR] + boxes[0][KD_CEIL]) / 2;
