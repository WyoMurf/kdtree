...
static int get_min_max(kd_list *list, int disc, long long *b_min, long long *b_max);
 ...
 */
{
    KDElem *loson, *hison;
    KDElem *lo, *eq, *hi;
    long long lo_min_bound, lo_max_bound, hi_min_bound, hi_max_bound;
    int num_lo, num_hi;
    int hort, tmp, m;
...
{
    KDElem *result;
    long long val; int new_disc, vert;
...
int kd_nearest(kd_tree tree, long long x, long long y, long long z, int m, kd_priority **alist);

void kd_print_nearest(kd_tree tree, long long x, long long y, long long z, int m)
{
	kd_priority *list;
	int xz,i;
...
static double coord_dist(long long x, long long  y)
{
        double d;
        d=x-y;
        d *= d;
	...
	while (realGen->top_index > 0)
	{
		...
		case KD_LOSON:
		        ...
		        if( Xq[d] <= p )
		        {
		                if( top_item->sons[KD_LOSON])
		                {
		                        long long old_Bn = top_elem->Bn[hort];
		                        long long old_Bp = top_elem->Bp[hort];
		                        if (vert)
		                        {
