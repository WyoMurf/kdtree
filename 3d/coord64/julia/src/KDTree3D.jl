...
mutable struct Node{T}
    ...
    function Node{T}(item, size::Box, lo::Int64, hi::Int64, ob::Int64) where T
...
mutable struct Tree{T}
    ...
    function Tree{T}() where T
        new{T}(nothing, 0, 0, zeros(Int64, 6))
    end
end
...
function nearest(tree::Tree{T}, x::Int64, y::Int64, z::Int64, m::Int) where T
    ...
    xq = (x, y, z, x, y, z)
    bp = [typemax(Int64) for _ in 1:6]
    bn = [typemin(Int64) for _ in 1:6]
...
function build_tree(items::Vector{T}, boxes::Vector{Box}, max_level::Int=100000) where T
    ...
    extent = [typemax(Int64) for _ in 1:6]
    mean = 0.0
    for i in 1:num
        b = boxes[i]
        for j in 1:3
