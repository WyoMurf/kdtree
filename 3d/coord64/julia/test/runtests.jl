...
@testset "KDTree3D basic tests" begin
    ...
    @test is_member(tree, 1, boxes[1])
    @test is_member(tree, 2, boxes[2])
    @test is_member(tree, 3, boxes[3])
    @test !is_member(tree, 4, (0,0,0,0,0,0))
    
    # Search
    results = search(tree, (5, 5, 5, 15, 15, 15))
    ...
    # Hard delete
    @test hard_delete!(tree, 1, boxes[1])
    @test count_items(tree) == 2
    @test !is_member(tree, 1, boxes[1])
    
    # Soft delete
    @test delete!(tree, 2, boxes[2])
    ...
    @test is_member(tree2, 1, new_boxes[1])
    @test is_member(tree2, 2, new_boxes[2])
    @test is_member(tree2, 3, new_boxes[3])
end
...
@testset "KDTree3D Really Delete & Badness & Nearest" begin
    ...
    for i in 1:10000
        ...
        z1 = rand(-100000:100000)
        x2 = x1 + rand(1:1000)
        y2 = y1 + rand(1:1000)
        z2 = z1 + rand(1:1000)
        b = (x1, y1, z1, x2, y2, z2)
        push!(boxes, b)
        insert!(tree, Int64(i), b)
    end
    ...
    for i in 10000:-1:1
        status, _, _ = really_delete!(tree, Int64(i), boxes[i])
        @test status == 1
    end
