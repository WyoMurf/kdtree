...
func main() {
	tree := kd.Create[int32]()
	rng := rand.New(rand.NewSource(42))

	nBoxes := 1000000
	boxes := make([]kd.Box[int32], nBoxes)

	fmt.Printf("Inserting %d boxes...\n", nBoxes)
	start := time.Now()
	for i := 0; i < nBoxes; i++ {
		x1 := rng.Int31n(1000000000)
		y1 := rng.Int31n(1000000000)
		z1 := rng.Int31n(1000000000)
		x2 := x1 + int32(rng.Intn(100)) + 1
		y2 := y1 + int32(rng.Intn(100)) + 1
		z2 := z1 + int32(rng.Intn(100)) + 1
		
		boxes[i] = kd.Box[int32]{x1, y1, z1, x2, y2, z2}
		tree.Insert(i+1, boxes[i])
	}
	fmt.Printf("Insertion took %v\n", time.Since(start))

	searchArea := kd.Box[int32]{0, 0, 0, 500000000, 500000000, 500000000}
