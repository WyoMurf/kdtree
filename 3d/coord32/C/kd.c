...
int kd_nearest(kd_tree tree, int x, int y, int z, int m, kd_priority **alist);

void kd_print_nearest(kd_tree tree, int x, int y, int z, int m)
{
	kd_priority *list;
	int xz,i;
...
static double coord_dist(int x, int  y)
{
        double d;
        d=x-y;
        d *= d;
	...
	while (realGen->top_index > 0)
	{
		...
		case KD_LOSON:
		        ...
		        if( Xq[d] <= p )
		        {
		                if( top_item->sons[KD_LOSON])
		                {
		                        int old_Bn = top_elem->Bn[hort];
		                        int old_Bp = top_elem->Bp[hort];
		                        if (vert)
		                        {
