...
pub struct Node<T> {
    pub item: Option<T>,
    pub size: KdBox,
    pub lo_min_bound: i32,
    pub hi_max_bound: i32,
    pub other_bound: i32,
...
impl<T: PartialEq + Clone> Tree<T> {
    ...
    ///
    /// ```
    /// use kdtree3d32::{Tree, KdBox};
    /// let mut tree = Tree::new();
    /// let bbox: KdBox = [0, 0, 0, 10, 10, 10];
    /// tree.insert("my_item", bbox);
    ...
    fn hard_delete_recursive(&mut self, node_idx_opt: Option<usize>, disc: usize, item: &T, size: &KdBox) -> Option<usize> {
        ...
        let is_match = match self.arena[node_idx].item {
        ...
        if is_match {
            ...
            if self.arena[node_idx].sons[1].is_some() {
                ...
                self.arena[node_idx].item = Some(q_item_clone.clone());
                self.arena[node_idx].size = q_size_clone;
                self.arena[node_idx].sons[1] = self.hard_delete_recursive(self.arena[node_idx].sons[1], next_disc(disc), &q_item_clone, &q_size_clone);
            } else {
                ...
                self.arena[node_idx].item = Some(q_item_clone.clone());
                self.arena[node_idx].size = q_size_clone;
                self.arena[node_idx].sons[0] = self.hard_delete_recursive(self.arena[node_idx].sons[0], next_disc(disc), &q_item_clone, &q_size_clone);
            }
            return Some(node_idx);
        }
    ...
    pub fn badness(&self) {
        ...
        fn stats<T>(tree: &Tree<T>, node_idx: Option<usize>, level: i32, factor3: &mut i32, max_levels: &mut i32) {
            if let Some(idx) = node_idx {
                let node = &tree.arena[idx];
                let has_lo = node.sons[0].is_some();
                let has_hi = node.sons[1].is_some();
                
                if (has_lo || has_hi) && !(has_lo && has_hi) { *factor3 += 1; }
    ...
    ///
    /// Returns a vector of `Priority` structures containing the distance and the item.
    pub fn nearest(&self, x: i32, y: i32, z: i32, m: usize) -> Vec<Priority<T>> {
    ...
    fn bounds_overlap_ball(&self, xq: &KdBox, bp: &KdBox, bn: &KdBox, m: usize, list: &[Priority<T>]) -> bool {
        ...
        for i in 0..3 {
            if xq[i] < bn[i] {
                let d = (xq[i] - bn[i]) as f64;
                sum += d * d;
                if sum > max_dist { return false; }
            } else if xq[i] > bp[i] {
                let d = (xq[i] - bp[i]) as f64;
                sum += d * d;
                if sum > max_dist { return false; }
            }
        }
...
fn kd_dist_sq(xq: &KdBox, box_size: &KdBox) -> f64 {
    ...
    if xq[LEFT] > box_size[RIGHT] {
        dx = (xq[LEFT] - box_size[RIGHT]) as f64;
    } else if xq[RIGHT] < box_size[LEFT] {
        dx = (box_size[LEFT] - xq[RIGHT]) as f64;
    }

    if xq[BOTTOM] > box_size[TOP] {
        dy = (xq[BOTTOM] - box_size[TOP]) as f64;
    } else if xq[TOP] < box_size[BOTTOM] {
        dy = (box_size[BOTTOM] - xq[TOP]) as f64;
    }

    if xq[FLOOR] > box_size[CEIL] {
        dz = (xq[FLOOR] - box_size[CEIL]) as f64;
    } else if xq[CEIL] < box_size[FLOOR] {
        dz = (box_size[FLOOR] - xq[CEIL]) as f64;
    }
            ...
            match top.state {
                ...
                State::LoSon => {
                    ...
                    if let Some(child_idx) = node.sons[0] {
                        ...
                        if m >= 3 {
                            if self.extent[hort] <= node.size[m] && self.extent[hort + 3] >= node.lo_min_bound { should_push = true; }
                        } else {
                            if self.extent[hort] <= node.other_bound && self.extent[hort + 3] >= node.lo_min_bound { should_push = true; }
                        }
                        if should_push {
                            self.stack.push(SimpleSave { node_idx: child_idx, disc: next_disc(m), state: State::ThisOne });
                            continue;
                        }
                    }
                }
                State::HiSon => {
                    ...
                    if let Some(child_idx) = node.sons[1] {
                        let mut should_push = false;
                        if m >= 3 {
                            if self.extent[hort] <= node.hi_max_bound && self.extent[hort + 3] >= node.other_bound { should_push = true; }
                        } else {
                            if self.extent[hort] <= node.hi_max_bound && self.extent[hort + 3] >= node.size[m] { should_push = true; }
                        }
                        if should_push {
                            self.stack.push(SimpleSave { node_idx: child_idx, disc: next_disc(m), state: State::ThisOne });
                            continue;
...
mod tests {
    ...
    fn rand_box(rng: &mut Lcg) -> KdBox {
        let left = (rng.next_range(RANGE_SPAN) as i32) + MIN_RANGE;
        let bottom = (rng.next_range(RANGE_SPAN) as i32) + MIN_RANGE;
        let floor = (rng.next_range(RANGE_SPAN) as i32) + MIN_RANGE;
        [left, bottom, floor, left + (rng.next_range(BOX_RANGE) as i32), bottom + (rng.next_range(BOX_RANGE) as i32), floor + (rng.next_range(BOX_RANGE) as i32)]
    }
    ...
    fn test_nearest() {
        ...
        for m in [1, 2, 4, 8, 16] {
            for _ in 0..50 {
                let qx = (rng.next_range(RANGE_SPAN) as i32) + MIN_RANGE;
                let qy = (rng.next_range(RANGE_SPAN) as i32) + MIN_RANGE;
                let qz = (rng.next_range(RANGE_SPAN) as i32) + MIN_RANGE;
