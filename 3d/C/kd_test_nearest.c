 ...
 */
static double box_dist(int qx, int qy, int qz, kd_box box)
{
...
int main(int argc, char **argv)
{
	...
	for (q = 0; q < NUM_QUERIES; q++) {
	    int qx = (rand() % RANGE_SPAN) + MIN_RANGE;
	    int qy = (rand() % RANGE_SPAN) + MIN_RANGE;
	    int qz = (rand() % RANGE_SPAN) + MIN_RANGE;
	    int visited;
