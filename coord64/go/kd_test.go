...
func randBox(rng *rand.Rand) Box {
	var b Box
	b[Left] = int64(rng.Intn(RANGE_SPAN_TEST) + MIN_RANGE_TEST)
	b[Bottom] = int64(rng.Intn(RANGE_SPAN_TEST) + MIN_RANGE_TEST)
	b[Right] = b[Left] + int64(rng.Intn(BOX_RANGE_TEST))
	b[Top] = b[Bottom] + int64(rng.Intn(BOX_RANGE_TEST))
	return b
}
...
func TestReallyDelete(t *testing.T) {
	...
	for i := KD_BOXES_TEST - 1; i >= 0; i-- {
		status, _, _ := tree.ReallyDelete(int64(i+1), boxes[i])
		if status != OK {
			if !tree.IsMember(int64(i+1), boxes[i]) {
...
func TestNearest(t *testing.T) {
	...
	for m := 1; m <= 20; m *= 2 {
		for q := 0; q < 50; q++ {
			qx := int64(rng.Intn(RANGE_SPAN_TEST) + MIN_RANGE_TEST)
			qy := int64(rng.Intn(RANGE_SPAN_TEST) + MIN_RANGE_TEST)
